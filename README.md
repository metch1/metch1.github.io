# MetcHi Portfolio

![Page Visits](https://visitor-badge.laobi.icu/badge?page_id=metch1.metch1)
![Twitter Follow](https://img.shields.io/twitter/follow/mettchi?style=social)
![License](https://img.shields.io/badge/License-All%20Rights%20Reserved-red)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![GitHub stars](https://img.shields.io/github/stars/metch1/metch1.github.io?style=social)

Welcome to my professional repository. This repository contains the full source code, designs, graphics, images, videos, and other assets used in my personal portfolio website.

## About

I am **MetcHi**, a Creative Designer and Video Editor with over 4 years of experience.  
This portfolio showcases my work in:

- Graphic Design (Gaming graphics, YouTube thumbnails, profile pictures, social media headers)
- Video Editing (Shorts, Reels, montages, music edits)
- Motion Graphics (UI animations, motion ads, visual effects)

You can view the live site here: [portfolio](https://metch1.github.io)

---

## License / Usage

**All Rights Reserved**.  

All content in this repository—including code, design, media, and assets—is the **intellectual property of Metch1**.  
You may **view the website publicly**, but you may **not copy, reuse, modify, distribute, or create derivative works** from this content without **explicit written permission**.

For inquiries, contact: **metchi.service@gmail.com**

---

## Notes

- This repository is meant to showcase my professional work and is protected under copyright law.  
- Any unauthorized use of the content may result in legal action.  

---

# TO DO 

- Convert all images to WebP for faster loading
- Compress videos for faster page load without losing quality
- Lazy-load images and videos for better performance
- Add favicon for browser tab
